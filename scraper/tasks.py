    ...
    for schedule in due_schedules:
        try:
            ...
            if not optimized_post:
                ...
                start_emoji = topic_emojis.get(schedule.topic, '✨')
                end_emoji = '🚀'
                optimized_post = f"{start_emoji} {optimized_post} {end_emoji}"
            ...
            # Trigger LinkedIn posting if credentials exist
            ...
            # Trigger Facebook/Meta posting if credentials exist
            if schedule.user.userprofile.meta_token and schedule.user.userprofile.meta_user_id:
                post_to_facebook.delay(scraped_content.id)
                logger.info(f"Triggered Facebook posting for content {scraped_content.id}")

            # Trigger Telegram posting if credentials exist
            if schedule.user.userprofile.telegram_bot_token and schedule.user.userprofile.telegram_chat_id:
                post_to_telegram.delay(scraped_content.id)
                logger.info(f"Triggered Telegram posting for content {scraped_content.id}")
...
def scrape_and_post(schedule_id):
    ...
    try:
        ...
        optimized_content = content_optimizer.optimize_content(
            f"Title: {title}\n\nContent: {content[:2000]}...",  # Send first 1000 chars for optimization
...
def post_to_facebook(content_id):
    ...
    content = ScrapedContent.objects.select_related('schedule__user__userprofile').get(id=content_id)
    profile = content.schedule.user.userprofile
    if not profile.meta_token or not profile.meta_user_id:
        logger.error("Meta credentials missing")
        return
    url = f"https://graph.facebook.com/{profile.meta_user_id}/feed"
    data = {
        'message': content.linkedin_post_text,  # Reuse or customize
        'access_token': profile.meta_token,
    }
    response = requests.post(url, data=data)
    response.raise_for_status()
    logger.info(f"Posted to Facebook: {response.json()}")


@shared_task(bind=True, max_retries=3)
def post_to_telegram(self, content_id):
    """Post the content to Telegram channels/groups"""
    logger.info(f"Starting post_to_telegram task for content ID: {content_id}")
    try:
        content = ScrapedContent.objects.select_related('schedule__user__userprofile').get(id=content_id)
        profile = content.schedule.user.userprofile
        
        if not profile.telegram_bot_token or not profile.telegram_chat_id:
            logger.error(f"Telegram credentials missing for content {content_id}")
            return
        
        # Prepare the post text
        post_text = content.linkedin_post_text  # Reuse LinkedIn text or customize
        
        # Get list of chat IDs to post to
        chat_ids = [chat_id.strip() for chat_id in profile.telegram_chat_id.split(',')]
        ...
        for chat_id in chat_ids:
            ...
            payload = {
                'chat_id': chat_id,
                'text': post_text,
                'parse_mode': 'HTML'  # Optional: supports basic HTML formatting
            }
        ...
        content.telegram_post_count = successful_posts
        content.posted_to_telegram = True if successful_posts > 0 else False
        content.save(update_fields=['telegram_post_count', 'posted_to_telegram'])
        
        logger.info(f"Successfully posted content ID: {content_id} to {successful_posts} Telegram chats")
        
    except requests.exceptions.RequestException as e:
        logger.error(f"Telegram API error for content {content_id}: {str(e)}")
        retry_in = (2 ** self.request.retries) * 60  # 1min, 2min, 4min
        raise self.retry(exc=e, countdown=retry_in)
    except Exception as e:
        logger.error(f"Unexpected error posting to Telegram for content {content_id}: {str(e)}")