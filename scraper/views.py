...
def account_settings(request):
    return render(request, 'scraper/account_settings.html')
...
def meta_auth(request):
    auth_url = (
        ...
        f"client_id={django_settings.META_CLIENT_ID}&"
        f"redirect_uri={django_settings.META_REDIRECT_URI}&"
...
def meta_callback(request):
    code = request.GET.get('code')
    if not code:
        messages.error(request, 'Meta authentication failed.')
        return redirect('dashboard')
    # Exchange code for access token
    response = requests.get(
        'https://graph.facebook.com/v19.0/oauth/access_token',
        params={
            'client_id': django_settings.META_CLIENT_ID,
            'redirect_uri': django_settings.META_REDIRECT_URI,
            'client_secret': django_settings.META_CLIENT_SECRET,
            'code': code,
        }
    )
    data = response.json()
    ...
    if access_token:
        request.user.userprofile.meta_token = access_token
        request.user.userprofile.save()
        messages.success(request, 'Meta account connected!')
    else:
        messages.error(request, 'Failed to get Meta access token.')
    return redirect('dashboard')
...
def meta_disconnect(request):
    ...
    profile.save()
    messages.success(request, "Meta account disconnected.")
    return redirect('dashboard')
...
def telegram_settings(request):
    profile = request.user.userprofile
    if request.method == 'POST':
        form = TelegramSettingsForm(request.POST, instance=profile)
        if form.is_valid():
            form.save()
            messages.success(request, 'Telegram settings updated successfully!')
            return redirect('dashboard')
    else:
        form = TelegramSettingsForm(instance=profile)

    context = {
        'form': form,
    }
    return render(request, 'scraper/telegram_settings.html', context)
...
def telegram_disconnect(request):
    ...
    profile.save()
    messages.success(request, "Telegram account disconnected.")
    return redirect('dashboard')